...
##' @noRd
##'
extractParams <- function (md, NMR, debug = 0){

	if (!NMR) {
		
		firstX <- grep("^\\s*##FIRSTX=", md)
		if (firstX == 0) stop("Couldn't find FIRSTX")
		firstX <- md[firstX]
		firstX <- sub("^\\s*##FIRSTX=", replacement = "", firstX)
		firstX <- gsub(",", ".", firstX) # for EU style files
		firstX <- as.numeric(firstX)
	
		lastX <- grep("^\\s*##LASTX=", md)
		if (lastX == 0) stop("Couldn't find LASTX")
		lastX <- md[lastX]
		lastX <- sub("^\\s*##LASTX=", replacement = "", lastX)
		lastX <- gsub(",", ".", lastX) # for EU style files
		lastX <- as.numeric(lastX)
	
		firstY <- grep("^\\s*##FIRSTY=", md)
		if (firstY == 0) stop("Couldn't find FIRSTY")
		firstY <- md[firstY]
		firstY <- sub("^\\s*##FIRSTY=", replacement = "", firstY)
		firstY <- gsub(",", ".", firstY) # for EU style files
		firstY <- as.numeric(firstY)
	
		npoints <- grep("^\\s*##NPOINTS=", md)
		if (npoints == 0) stop("Couldn't find NPOINTS")
		npoints <- md[npoints]
		npoints <- sub("^\\s*##NPOINTS=", replacement = "", npoints)
		npoints <- as.integer(npoints)
	
		factorY <- grep("^\\s*##YFACTOR=", md)
		if (factorY == 0) stop("Couldn't find YFACTOR")
		factorY <- sub("^\\s*##YFACTOR=", replacement = "", md[factorY])
		factorY <- gsub(",", ".", factorY) # for EU style files
		factorY <- as.numeric(factorY)
		
		factorX <- grep("^\\s*##XFACTOR=", md)
		if (factorX == 0) stop("Couldn't find YFACTOR")
		factorX <- sub("^\\s*##XFACTOR=", replacement = "", md[factorX])
