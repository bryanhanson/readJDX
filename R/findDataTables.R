...
findDataTables <- function (jdx, debug = 0) {
	...
	ST_pats <- c("^\\s*##XYDATA\\s*=\\s*\\(X\\+\\+\\(Y\\.\\.Y\\)\\)$", # IR, typically
	...
	END_pats <- c("^\\s*##END\\s*=", # IR, typically
		"^\\s*##PAGE\\s*=\\s*N=2", # real NMR data
		"^\\s*##END\\s{1}NTUPLES\\s*=", # imaginary NMR data
		# "\\$\\$\\s{1}checkpoint", # 2D NMR data (real) JEOL style, keeps to JCAMP standard
		"^\\s*##PAGE\\s*=\\s*F1=") # 2D NMR data
