...
\begin{enumerate}
  ...
  \item Line number reporting during debugging is slightly off for NMR files that have lines which are solely composed of a comment. These start with "\$\$" and are deleted during processing.
\end{enumerate}
